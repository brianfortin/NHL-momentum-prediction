...
A machine-learning project that quantifies the impact of team-level variables on NHL game outcomes including *Team Momentum*, *Opponent Strength*, and *Schedule Fatigue*.
...
* **Scope:** 191 games (2022-2023 Historic Season + 2023-2024 Season).

### 2. Feature Engineering
I isolated specific explanatory variables to test how game context affects outcome. The high-level concepts used in the hypothesis map to the following engineered features:
...
**Metric**
The ROC curve plots true positive rate against false positive rate, and its area (AUC) measures how well the model ranks positive outcomes above negative ones.
...
|:------------------------|:---------|:------|:----------------------------------------------------------------------------------------------------------------------------|
| **Logistic Regression** | 56.4%    | N/A   | Baseline. Struggled with non-linear relationships.                                                                          |
| **XGBoost**             | 59.4%    | 0.606 | Final model. Tree-based logic successfully modeled non-linear patterns, achieving a **3% accuracy lift** over the baseline. |

### Modeling
I compared the *Logistic Regression* (linear baseline) model against the *XGBoost* (gradient-boosted trees) model.
The results demonstrated that game outcomes are driven by non-linear interactions between *Team Momentum*, *Opponent Strength*, and *Schedule Fatigue*, which the linear model failed to fully capture.
...
2.  **Algorithm Selection Validated:** The 3% increase in model accuracy by the *XGBoost* model over the *Logistic Regression* baseline validates the necessity of using non-linear models to capture complex interactions in sports data.
